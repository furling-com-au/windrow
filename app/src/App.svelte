...
<!-- the narrator is the running commentary on what the map is doing; without a live region
     ...
     It stays mounted for that reason (opacity:0 via class:hidden, not {#if}) — m-hide
     additionally hides it on mobile when the sheet is expanded or a site card is open
     (#30), where it would otherwise sit on top of them. -->
<div
  class="narrator"
  class:shift={app.viewMode === "advanced"}
  class:hidden={!narratorText}
