...
export const SCENARIOS: { id: ScenarioId; label: string; story: string; levers: Partial<Levers> }[] = [
  ...
  { id: "roadclosure", label: "Tod Highway closed", story: "The peninsula's central spine is impassable, from Port Lincoln to Wudinna. Trucks re-sort toward Lincoln Highway sites, and the silo-to-port shuttle detours with them. Every leg is charged 2.5× on the share of it that ran on the closed highway — an ASSUMED detour penalty, not a re-routed path: the model knows which legs used the road, not what going around one actually costs.", levers: { roadClosure: true } },
];
