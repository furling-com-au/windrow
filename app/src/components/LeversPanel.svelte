...
<div class="levers">
  ...
  {#if app.viewMode === "advanced"}
    ...
    <InfoTip
      ...
      text="Bring back trains: two 1,600 t trainsets return to the railway closed in 2019 (Cummins/Kimba/Wudinna into Port Lincoln), replacing a third of the road shuttle. No timetable — they run when port stocks need topping up, at narrow-gauge line speed rather than road speed, which works out at ~1.5 cycles a day each (assumption A21); how much trucking this saves depends strongly on the silo-to-port load assumption below. · Port closed 1 wk: closes the Port Lincoln terminal for 7 days at harvest peak (mid-December). · Tod Hwy closed: closes the Tod Highway, the peninsula's central spine, Port Lincoln to Wudinna. Farm and silo-to-port legs alike are charged an assumed 2.5x on the share of each leg that ran on the closed road — the detour is a penalty, not a re-routed path."
    />
